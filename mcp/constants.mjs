export const SERVER_NAME = "Cowart MCP";
export const SERVER_VERSION = "0.7.0";
export const TOOL_GET_SELECTION = "get_cowart_selection";
export const TOOL_INSERT_IMAGE = "insert_cowart_image";
export const TOOL_GET_CANVAS = "get_cowart_canvas";
export const TOOL_GET_ANNOTATIONS = "get_cowart_annotations";
export const TOOL_CREATE_HOLDER = "create_cowart_image_holder";
export const TOOL_REPLACE_IMAGE = "replace_cowart_image";
export const TOOL_EXPORT_VIEW = "export_cowart_view";
export const TOOL_ADD_SHAPES = "add_cowart_shapes";
export const TOOL_MAKE_MASK = "make_cowart_mask";
export const TOOL_UPDATE_HOLDER = "update_cowart_holder";
export const TOOL_GET_REFERENCES = "get_cowart_references";
export const TOOL_GET_REQUESTS = "get_cowart_requests";
export const TOOL_SEGMENT_IMAGE = "segment_cowart_image";
export const TOOL_REFINE_SEGMENT = "refine_cowart_segment";
export const TOOL_EXTRACT_OBJECT = "extract_cowart_object";
export const TOOL_CREATE_VARIANT_GRID = "create_cowart_variant_grid";
export const TOOL_SELECT_VARIANT = "select_cowart_variant";
export const TOOL_CREATE_DECOMPOSITION = "create_cowart_decomposition";
export const TOOL_PUBLISH_DECOMPOSITION_ARTIFACT = "publish_cowart_decomposition_artifact";
export const AI_IMAGE_HOLDER_STATUSES = ["empty", "requested", "generating", "failed", "filled"];
export const AI_IMAGE_HOLDER_LABEL = "AI 图片";
export const AI_IMAGE_HOLDER_DEFAULT_W = 320;
export const AI_IMAGE_HOLDER_DEFAULT_H = 220;
export const COWART_AI_IMAGE_SHAPE = "cowart-ai-image";
export const PAGE_ID_PREFIX = "page:";
export const PAGE_ASSETS_ROUTE = "/page-assets/";
