...
> 日期：2026-07-27。状态：Phase 8.1-8.4 已落地，包括对象动作、本地 Sidecar 与混合场景分解。

## 1. 当前交付边界

Phase 8.1-8.4 已交付“人在回路”的对象选择、校正、动作、自动分割和 agent 消费流程：
...
当前版本交付 loopback Sidecar、text/automatic segmentation 与轻量 AI scene decomposition；不交付完整 PSD 图层恢复、文字恢复、生产级 C2PA 或 video。

## 2. 实现事实

- MCP server：`0.7.0`。
- 公开 MCP 工具数：`19`。
- 对象相关工具：`segment_cowart_image`、`refine_cowart_segment`、`make_cowart_mask({ segmentId })`、`extract_cowart_object`、`create_cowart_variant_grid`、`select_cowart_variant`、`create_cowart_decomposition`、`publish_cowart_decomposition_artifact`。
...
未配置 Sidecar 时返回 `browser_interaction_required`。配置 loopback Sidecar 后支持 point、box、text 和 automatic，但默认只返回候选；只有调用方显式传入 `publish=true` 才能写入 Segment Store。
...
`extract_cowart_object` 只使用 Segment Store 权威蒙版和当前源图像素。原图对象层标记 `synthetic: false`；从 AI 候选二次分割并提取的对象层继承 `synthetic: true`。`preserveOutside` 从已验证 segment 推导 source identity，并保证 mask 值为 0 的解码 RGBA 字节与源图一致，不允许 caller 覆盖 source shape、asset、hash 或 mask hash。

## 5. 隐私、安全和限制

- 普通对象分割只从 Cowart localhost 页面资源读取，不上传源图。只有用户逐次确认的 scene decomposition 才将选定源图交给 Codex `image_gen`。
...
已记录证据覆盖固定 fixture 的真实浏览器点选、scribble、确认、reload rehydrate、keyboard accept/cancel、unsupported、checksum error、stale source，以及 crop/flip alignment。Sidecar smoke 在本地生成小图上验证 point、text 和 automatic 的真实模型数据流与资源峰值；这些结果不能外推为通用质量 benchmark。
...
### Phase 8.3：本地 provider（已落地）

本地 Sidecar 支持 point、box、text 和 automatic。Sidecar 默认绑定 `127.0.0.1`，不启用 CORS，使用 `0600` token，默认单任务且无云端降级。MCP 只有 `publish=true` 才持久化候选。

### Phase 8.4：混合场景分解（已落地）

每次上传必须明确确认；默认只生成 depth hint 与 clean plate。真实可见对象层来自 Segment Store；AI 生成 artifact 标记 synthetic，并由 Decomposition Stack 轻量编排。
遮挡补全按对象显式请求；生成候选必须经 Sidecar SAM 2 二次分割与透明提取，发布 `completed_object` 时记录 `generatedSegmentId`。
