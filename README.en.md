...
- Use Cowart MCP tools to perceive and act on the canvas: read selection state and structured canvas content, parse annotations (text and the shape each points at), insert images, create AI image holders, replace images, author shapes (text/sticky/arrow/geometry; arrows can bind to nodes for follow-on-move flowchart connectors), export views to image files, and save page-local assets.
