...
- 通过 Cowart MCP 工具感知与操作画布：读取选择状态与结构化画布内容、解析批注（文本与指向目标）、插入图片、创建 AI image holder、替换图片、创建图形（文本/便签/箭头/几何，箭头可绑定到节点形成可跟随的流程图连接）、为局部重绘生成蒙版、记录图片修订血缘（版本间连接）、导出视图为图片文件，并保存到页面本地资源目录（AI 图片 holder 是自带状态的自定义形状）。
